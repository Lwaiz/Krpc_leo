...
class UserService : public Kuser::UserServiceRpc{
    ...
    /*
     * 重写基类 UserServiceRpc 的虚函数，这些方法倍 RPC 框架直接调用
     * 1.调用者(caller)通过 RPC 框架发送 Login请求
     * 2.服务提供者(callee) 接收到请求后，调用者下面重写的 Login 方法
     */
